...
def _load_report_config(config_data: Dict) -> Dict:
    ...
    auto_update_freq_env = _get_env_bool("AUTO_UPDATE_FREQUENCY_WORDS")
    ...
    return {
        ...
        # 频率词自动更新配置
        "AUTO_UPDATE_FREQUENCY_WORDS": auto_update_freq_env if auto_update_freq_env is not None else report_config.get("auto_update_frequency_words", False),
        "FREQUENCY_UPDATE_DAYS": _get_env_int("FREQUENCY_UPDATE_DAYS") or report_config.get("frequency_update_days", 7),
        "FREQUENCY_SCORE_THRESHOLD": float(_get_env_str("FREQUENCY_SCORE_THRESHOLD") or report_config.get("frequency_score_threshold", 0.4)),
        "FREQUENCY_MAX_APPEND": _get_env_int("FREQUENCY_MAX_APPEND") or report_config.get("frequency_max_append", 10),
    }
...
def _load_storage_config(config_data: Dict) -> Dict:
    ...
    return {
        ...
        "FORMATS": {
            ...
            "MARKDOWN_OUTPUT_DIR": _get_env_str("MARKDOWN_OUTPUT_DIR") or formats.get("markdown_output_dir", "page/src/content/post"),
        },
        "LOCAL": {
