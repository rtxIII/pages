...
from trend.storage.base import StorageBackend, NewsItem, NewsData
from trend.utils.time import (
    get_configured_time,
    format_date_folder,
...
class LocalStorageBackend(StorageBackend):
    ...
    def get_historical_data(self, days: int = 7) -> Dict[str, NewsData]:
        """
        获取过去N天的历史数据
        ...
        Returns:
            {date: NewsData} 字典,按日期组织的历史数据
        """
        historical_data = {}
        
        try:
            current_time = self._get_configured_time()
            ...
            print(f"[本地存储] 成功加载 {len(historical_data)} 天的历史数据")
            return historical_data
            
        except Exception as e:
            print(f"[本地存储] 获取历史数据失败: {e}")
            return {}
