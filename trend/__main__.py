...
from trend.context import AppContext

# 版本号直接定义，避免循环导入
VERSION = "4.0.0"
from trend.core import load_config
from trend.crawler import DataFetcher
from trend.storage import convert_crawl_results_to_news_data
...
class NewsAnalyzer:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # 频率词自动更新
            if self.ctx.config.get("AUTO_UPDATE_FREQUENCY_WORDS", False):
                try:
                    from trend.core.frequency_updater import update_frequency_words
                    
                    frequency_file = os.environ.get("FREQUENCY_WORDS_PATH", "config/frequency_words.txt")
                    days = self.ctx.config.get("FREQUENCY_UPDATE_DAYS", 7)
                    threshold = self.ctx.config.get("FREQUENCY_SCORE_THRESHOLD", 0.6)
                    max_append = self.ctx.config.get("FREQUENCY_MAX_APPEND", 10)
                    
                    update_frequency_words(
                        storage_backend=self.storage_manager,
            ...
            if self.ctx.config["STORAGE"]["FORMATS"].get("MARKDOWN", False):
                try:
                    from trend.report.markdown import generate_markdown_from_analysis
                    
                    # 加载分析数据
                    analysis_data = self._load_analysis_data()
                    if analysis_data:
                        all_results, id_to_name_map, title_info, new_titles, word_groups, filter_words, global_filters = analysis_data
                        
                        # 运行分析获取统计数据
                        stats, _ = self._run_analysis_pipeline(
                            all_results,
                            "daily",
                            title_info,
                            new_titles,
                        ...
                        # 生成 Markdown
                        output_dir = self.ctx.config["STORAGE"]["FORMATS"].get("MARKDOWN_OUTPUT_DIR", "page/src/content/post")
                        md_file = generate_markdown_from_analysis(
                            stats=stats,
