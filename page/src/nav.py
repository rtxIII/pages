...
with open('nav.yml', 'r') as f:
    html = ""
    doc = yaml.load(f, Loader=yaml.FullLoader)
    for title in doc['titles']:
        title = title['title']
        html = html + f'''<div class="span11"><h2 class="">{title['name']}</h2> <hr class="soften">'''
        items = title['items']
        if doc['sorted']:
            items = sorted(items, key=lambda x: len(x['name']), reverse=True)

        for item in items:
            html = html + f'''<a class="span3" href="{item['url']}"><p>{item['logo']} {item['name']}</p></a>'''
        html = html + '</div>'
