...
if [[ -n $INPUT_ICON ]]; then
  icon="$INPUT_ICON"
  echo -e "${cyan}Use custom icon ${icon}"
else
...
request_body="title=${INPUT_TITLE}&body=${INPUT_BODY}&sound=${INPUT_SOUND}&isArchive=${INPUT_IS_ARCHIVE}&url=${INPUT_URL}&automaticallyCopy=${INPUT_AUTOMATICALLY_COPY}&copy=${INPUT_COPY}&icon=${icon}&group=${INPUT_GROUP}"
